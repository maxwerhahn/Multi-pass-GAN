...
useObstacles   = int(ph.getParam( "useObstacles",   0  )) 			# use obstacles or not(levelsets)
...
if ((not useTempoD) and (not useTempoL2)): # should use the full sequence, not use multi_files
	...
	floader = FDL.FluidDataLoader( print_info=1, base_path=packedSimPath, filename=lowfilename, oldNamingScheme=False, filename_y=None, filename_index_max=frame_max, indices=dirIDs, data_fraction=data_fraction, multi_file_list=mfl, multi_file_list_y=mfh)
else:
	lowparalen = len(mfl)
	highparalen = len(mfh)
...
#	cut out
x = x[:,:,:,startIndex:startIndex+simSizeLow,:]
#y = y[:,:,:,startIndex*upRes:(startIndex+simSizeLow)*upRes,:]
#x = np.flip(x, axis = 2)
#y = np.flip(y, axis = 2)
...
def gen_resnet(_in, reuse=False, use_batch_norm=False, train=None):
	...
	with tf.variable_scope("generator", reuse=reuse) as scope:
		...
		#check tanh here?
		#resF1 = ( tf.add( gc7[0] , inp ) )
		#resF = tf.reshape( resF1, shape=[-1, tileSizeHigh*tileSizeHigh] )
		#gan.flatten()
		#return gan.y()
...
def getinput(index = 1, randomtile = True, isTraining = True, batch_size = 1, useDataAugmentation = False, modifyvelocity = False, useVelocities = False, useVorticities = False, useObstacles = False, useFlags = False):
	if randomtile == False:
	...
	if useFlags:		
		Obsinput = batch_xts[:,:,:,:,4:5] 
		for i in range(0, real_batch_sz):
			for j in range(0, tileSizeLow):
				for k in range(0, tileSizeLow):
					if Obsinput[i][0][j][k][0] <= 3 and Obsinput[i][0][j][k][0] >= 1.5:
						Obsinput[i][0][j][k][0] = 1.0
					else:
						Obsinput[i][0][j][k][0] = 0.0
					
		densVel = batch_xts[:,:,:,:,0:4] 
		batch_xts = np.concatenate((densVel, Obsinput), axis = 4)
	...
	if useVelocities and modifyvelocity:
		...
		if useObstacles:
			Obsinput = batch_xs[:,:,:,:,4:5]
		if useFlags and useObstacles:
			Flaginput = batch_xs[:,:,:,:,5:6]
		else:
		...
		if useObstacles:
			batch_xs = np.concatenate((batch_xs, Obsinput), axis = 4)
		if useFlags:
...
def getTempoinput(batch_size = 1, isTraining = True, useDataAugmentation = False, useVelocities = False, useVorticities = False, n_t = 3, dt=1.0, useFlags = False):
...
def generateTestImage(sim_no = fromSim, frame_no = 0, outPath = test_path,imageindex = 0, modifyvelocity = False):
	if premadeTiles:
		...
		batch_xs, batch_ys = getinput(randomtile = False, index = (sim_no-fromSim)*frame_max + frame_no, modifyvelocity = modifyvelocity, useVelocities = useVelocities, useVorticities = useVorticities, useObstacles = useObstacles, useFlags = useFlags)

		batch_xts = np.reshape(batch_xs, (-1,1,tileSizeLow,tileSizeLow,n_inputChannels))
		batch_yts = np.reshape(batch_ys, (-1,1,tileSizeHigh,tileSizeHigh,1))
		tc.savePngsBatch(batch_xts, batch_yts, tiCr, outPath, tileSize = tileSizeLow)
		resultTiles = []
		...
		tc.savePngsGrayscale(np.reshape(batch_ys_in,[batch_xs_in.shape[0],int(imgSz),int(imgSz), 1]),outPath, imageCounter=300+imageindex, tiles_in_image=tiles_in_image)

def generate3DUni(sim_no = fromSim, frame_no = 1, outPath = test_path,imageindex = 0):
	if dataDimension == 2:
		print("only for 3D Uni files output!")	
		exit(1)
	if premadeTiles:
		#todo output for premadetiles
		pass
	else:
		if (overlappedpixel*2 > tileSize) or (tileSize > simLowLength):
			print("Wrong parameters for 3d output!")	
			exit(1)
		batch_xs, _ = tiCr.getDatum((sim_no-fromSim)*frame_max + frame_no)
		if useVelocities and changeVelocity:
		...
		lengthnum = ((simLowLength-overlappedpixel*2+tileSize-overlappedpixel*2-1)//(tileSize-overlappedpixel*2))
		widthnum = ((simLowWidth-overlappedpixel*2+tileSize-overlappedpixel*2-1)//(tileSize-overlappedpixel*2))
		heightnum = ((simLowHeight-overlappedpixel*2+tileSize-overlappedpixel*2-1)//(tileSize-overlappedpixel*2))

		for i in range(lengthnum):
			for j in range(widthnum):
				for k in range(heightnum):
					ifrom = (tileSize-overlappedpixel*2)*i
					ito = (tileSize-overlappedpixel*2)*i+tileSize
					jfrom = (tileSize-overlappedpixel*2)*j
					jto = (tileSize-overlappedpixel*2)*j+tileSize
					kfrom = (tileSize-overlappedpixel*2)*k
					kto = (tileSize-overlappedpixel*2)*k+tileSize
					if ito >simLowLength:
						ifrom = simLowLength-tileSize
						ito = simLowLength
					if jto >simLowWidth:
						jfrom = simLowWidth-tileSize
						jto = simLowWidth
					if kto >simLowHeight:
						kfrom = simLowHeight-tileSize
						kto = simLowHeight
		...
		high = np.zeros([simLowLength*upres,simLowWidth*upres,simLowHeigh*upres])

		for i in range(lengthnum):
			for j in range(widthnum):
				for k in range(heightnum):
					ihighfrom = (tileSize-overlappedpixel*2)*upres*(i-1)+(tileSize-overlappedpixel)*upres
					ihighto = ihighfrom + (tileSize-overlappedpixel*2)*upres
					jhighfrom = (tileSize-overlappedpixel*2)*upres*(j-1)+(tileSize-overlappedpixel)*upres
					jhighto = jhighfrom+(tileSize-overlappedpixel*2)*upres
					khighfrom = (tileSize-overlappedpixel*2)*upres*(k-1)+(tileSize-overlappedpixel)*upres
					khighto = khighfrom+(tileSize-overlappedpixel*2)*upres
					ifrom = overlappedpixel*upres
					ito = (tileSize-overlappedpixel)*upres
					jfrom = overlappedpixel*upres
					jto = (tileSize-overlappedpixel)*upres
					kfrom = overlappedpixel*upres
					kto = (tileSize-overlappedpixel)*upres
					if i == 0:
						ifrom = 0
						ito = (tileSize-overlappedpixel)*upres
						ihighfrom = 0
						ihighto = (tileSize-overlappedpixel)*upres
					if j == 0:
						jfrom = 0
						jto = (tileSize-overlappedpixel)*upres
						jhighfrom = 0
						jhighto = (tileSize-overlappedpixel)*upres
					if k == 0:
						kfrom = 0
						kto = (tileSize-overlappedpixel)*upres
						khighfrom = 0
						khighto = (tileSize-overlappedpixel)*upres
					if i == lengthnum-1:
						ifrom = overlappedpixel*upres
						ito = tileSize*upres
						ihighfrom = simLowLength*upres-tileSize*upres
						ihighto = simLowLength*upres
					if j == widthnum-1:
						jfrom = overlappedpixel*upres
						jto = tileSize*upres
						jhighfrom = simLowWidth*upres-tileSize*upres
						jhighto = simLowWidth*upres
					if k == heightnum-1:
						kfrom = overlappedpixel*upres
						kto = tileSize*upres
						khighfrom = simLowHeigh*upres-tileSize*upres
						khighto = simLowHeigh*upres
		...
		high = np.reshape(high,[simLowLength*upres,simLowWidth*upres,simLowHeigh*upres])
		
		head, _ = uniio.readUni(packedSimPath + "../../3ddata_sim_uni/sim_%04d/density_high_%04d.uni"%(sim_no))
		head['dimX'] = simLowLength*upres
		head['dimY'] = simLowWidth*upres
		head['dimZ'] = simLowHeigh*upres
		uniio.writeUni(outPath+'source_%04d.uni'%(frame_no+frame_min), head, high)
		if(0): # Rachel save png
...
cost = 0.0
# batch_xs, batch_ys = getinput(randomtile = True, batch_size = 16, useDataAugmentation = useDataAugmentation, useVelocities = useVelocities, useVorticities = useVorticities, useObstacles = useObstacles, useFlags = useFlags)
# batch_xs = np.reshape(batch_xs, (-1,1,tileSizeLow,tileSizeLow,n_inputChannels))
# batch_ys = np.reshape(batch_ys, (-1,1,tileSizeHigh,tileSizeHigh,1))
# tc.savePngsBatch(batch_xs, batch_ys, tiCr, 'D:/b/', tileSize = tileSizeLow)

# batch_xs, batch_ys, _ = getTempoinput(batch_size_disc* 10, n_t = 3, dt=0.5, useVelocities = useVelocities, useVorticities = useVorticities, useDataAugmentation = useDataAugmentation, useFlags = useFlags)
# batch_xs = np.reshape(batch_xs, (-1,1,tileSizeLow,tileSizeLow,n_inputChannels))
# batch_ys = np.reshape(batch_ys, (-1,1,tileSizeHigh,tileSizeHigh,1))
# tc.savePngsBatch(batch_xs, batch_ys, tiCr, 'D:/b/', tileSize = tileSizeLow)
# in_xs = np.reshape(batch_xs, (-1, tileSizeLow, tileSizeLow, n_inputChannels))
# indices = []
# for i in range(0,tileSizeLow):
	# for j in range(0,tileSizeLow):
		# if in_xs[0][i][j][4] != 0:
			# indices.append((i,j))
			# #print(in_xs[0][i][j][4])
		...
		for epoch in range(trainingEpochs):
			...
			if use_spatialdisc:
				for runs in range(discRuns):
					batch_xs, batch_ys = getinput(batch_size = batch_size_disc, useDataAugmentation = useDataAugmentation, useVelocities = useVelocities, useVorticities = useVorticities, useObstacles = useObstacles, useFlags = useFlags)
			...
			if(useTempoD):
				for runs in range(discRuns):
					batch_xts, batch_yts, batch_y_pos = getTempoinput(batch_size_disc, n_t = 3, dt=0.5, useVelocities = useVelocities, useVorticities = useVorticities, useDataAugmentation = useDataAugmentation, useFlags = useFlags)
			...
			if (epoch + 1) % testInterval == 0:
				if use_spatialdisc:
					...
					batch_xs, batch_ys = getinput(batch_size = numTests, useVelocities = useVelocities, useVorticities = useVorticities, useObstacles = useObstacles, useFlags = useFlags)
					...
					# testing starts here...
					# get test data
					batch_xs, batch_ys = getinput(batch_size = numTests, isTraining=False, useVelocities = useVelocities, useVorticities = useVorticities, useObstacles = useObstacles, useFlags = useFlags)
				...
				if(useTempoD): # temporal logs
					...
					batch_xts, batch_yts, batch_y_pos = getTempoinput(numTests, useVelocities = useVelocities, useVorticities = useVorticities, n_t = 3, dt=0.5, useFlags = useFlags)
					...
					# test data
					batch_xts, batch_yts, batch_y_pos = getTempoinput(numTests, isTraining=False, useVelocities = useVelocities, useVorticities = useVorticities, n_t = 3, dt=0.5, useFlags = useFlags)
...
### OUTPUT MODE ###
	...
	for layerno in range(0,frame_max):
		if dataDimension == 2:
