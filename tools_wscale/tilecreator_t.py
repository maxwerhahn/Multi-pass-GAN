...
C_LAYOUT = {
	'dens':C_KEY_DEFAULT,
	'dens_vel':'d,vx,vy,vz',
	'dens_vel_obs':'d,vx,vy,vz,o',
	'dens_vel_obs_flags':'d,vx,vy,vz,o,f',
	'dens_vel_flags':'d,vx,vy,vz,f'
	}

class TileCreator(object):

	def __init__(self, tileSizeLow, simSizeLow=64, upres=2, dim=2, dim_t=1, overlapping=0, densityMinimum=0.02, premadeTiles=False, partTrain=0.9, partTest=0.1, partVal=0, channelLayout_low=C_LAYOUT['dens_vel'], channelLayout_high=C_LAYOUT['dens'], highIsLabel=False, loadPN=False, padding=0):
		'''
		...
		self.upres = np.asarray(upres)
		if np.isscalar(upres):
			self.upres = upres
		elif self.dim==2 and len(upres) == 2:
			self.upres = [1] + upres
		elif len(upres) == 3 and self.dim==3:
			self.upres = upres
		if np.isscalar(upres):
			if self.upres < 1:
		...
		if len(self.c_lists[DATA_KEY_LOW][C_KEY_OBSTACLE])>0: 
			print('  obstacle channels: {}'.format(self.c_lists[DATA_KEY_LOW][C_KEY_OBSTACLE]))
		if len(self.c_lists[DATA_KEY_LOW][C_KEY_FLAGS])>0: 
			print('  flags channels: {}'.format(self.c_lists[DATA_KEY_LOW][C_KEY_FLAGS]))
		if len(self.c_lists[DATA_KEY_LOW][C_KEY_EPS])>0: 
			print('  flags channels: {}'.format(self.c_lists[DATA_KEY_LOW][C_KEY_EPS]))
		if len(self.c_lists[DATA_KEY_LOW][C_KEY_K])>0: 
			print('  flags channels: {}'.format(self.c_lists[DATA_KEY_LOW][C_KEY_K]))
		...
		if len(self.c_lists[DATA_KEY_HIGH][C_KEY_OBSTACLE])>0: 
			print('  obstacle channels: {}'.format(self.c_lists[DATA_KEY_HIGH][C_KEY_OBSTACLE]))
		if len(self.c_lists[DATA_KEY_HIGH][C_KEY_FLAGS])>0: 
			print('  flags channels: {}'.format(self.c_lists[DATA_KEY_HIGH][C_KEY_FLAGS]))
		...
		self.data_flags = {
			DATA_KEY_LOW:{
				...
				C_KEY_OBSTACLE:len(self.c_lists[DATA_KEY_LOW][C_KEY_OBSTACLE])>0,
				C_KEY_FLAGS:len(self.c_lists[DATA_KEY_LOW][C_KEY_FLAGS])>0,
				C_KEY_K:len(self.c_lists[DATA_KEY_LOW][C_KEY_K])>0,
				C_KEY_EPS:len(self.c_lists[DATA_KEY_LOW][C_KEY_EPS])>0,
			...
			DATA_KEY_HIGH:{
				...
				C_KEY_OBSTACLE:len(self.c_lists[DATA_KEY_HIGH][C_KEY_OBSTACLE])>0,
				C_KEY_FLAGS:len(self.c_lists[DATA_KEY_HIGH][C_KEY_FLAGS])>0,
				C_KEY_K:len(self.c_lists[DATA_KEY_HIGH][C_KEY_K])>0,
				C_KEY_EPS:len(self.c_lists[DATA_KEY_HIGH][C_KEY_EPS])>0,
				C_KEY_POSITION:False
			}
		}
		if loadPN:
	...
	def addData(self, low, high, flip_vel_z=True ):
		...
		# NT_DEBUG, TODO distinguish dimt set and no dimt?
	...
	def getRandomTile(self, low, high, tileShapeLow=None, bounds=[0,0,0,0]): #bounds to avoid mirrored parts
		...
		if 0:
			if tileShapeLow is None:
				...
				tileShapeHigh= np.copy(self.tile_shape_high)
			else:
				tileShapeHigh = tileShapeLow
				tileShapeHigh *= self.upres
				
		if tileShapeLow is None:
			tileShapeLow = np.copy(self.tile_shape_low) # use copy is very important!!!
		if np.isscalar(self.upres):
			tileShapeHigh = tileShapeLow*self.upres
		else:
			tileShapeHigh = tileShapeLow * np.array((1,1,8,1))
		# print(tileShapeLow)
		# print(tileShapeHigh)
		...
		#boundaries
		#print(bounds)
		start = np.floor(bounds)#.astype(int)
		...
		while (not hasMinDensity) and i<20:
			...
			if not self.data_flags[DATA_KEY_HIGH]['isLabel']:
				...
				#highTile = high[offset[0]:offset[0]+tileShapeHigh[0], offset[1]:offset[1]+tileShapeHigh[1], offset[2]:offset[2]+tileShapeHigh[2], :]
			else:
				highTile = high
...
#####################################################################################
	...
	def rotate(self, data):
		...
		#3D:
			...
			#https://www.lfd.uci.edu/~gohlke/code/transformations.py.html -> quaternion_matrix()
...
#####################################################################################
	...
	def parseChannels(self, channelString):
		...
		self.parse = {
			...
			C_KEY_OBSTACLE:self.parseCObstacle,
			C_KEY_FLAGS:self.parseCFlags,
			C_KEY_K:self.parseCK,
			C_KEY_EPS:self.parseCEps,
	...
	def parseCObstacle(self, c, i, c_types):
		if c[i]=='o':
			c_types[C_KEY_OBSTACLE].append(i)
		else:
			self.TCError('channel {}: unknown channel key \"{}\".'.format(i, c[i]))
			
	def parseCFlags(self, c, i, c_types):
		if c[i]=='f':
			c_types[C_KEY_FLAGS].append(i)
		else:
			self.TCError('channel {}: unknown channel key \"{}\".'.format(i, c[i]))
			
	def parseCK(self, c, i, c_types):
		if c[i]=='k':
			c_types[C_KEY_K].append(i)
		else:
			self.TCError('channel {}: unknown channel key \"{}\".'.format(i, c[i]))
			
	def parseCEps(self, c, i, c_types):
		if c[i]=='e':
			c_types[C_KEY_EPS].append(i)
		else:
			self.TCError('channel {}: unknown channel key \"{}\".'.format(i, c[i]))
...
batchCounterGlob=0
def savePngsBatch(low, high, TC, path, batchCounter=-1, save_vels=False, dscale=1., vscale=1., tileSize = 16, upRes=4):
	...
	for axis in range(1 if dim==2 else 3):
		suff = aNames[axis]
		# NT_DEBUG rename to xy etc.
		if dim == 3:
			highD = np.average(high, axis=axis+1) * dscale
			lowD  = np.average(low, axis=axis+1) * dscale
			#suff = 'a{:02d}_'.format(axis)
		if dim == 2:
			highD = high*dscale ; lowD = low * dscale
			lowD.shape = (batch, tileSize, tileSize, low.shape[4])
			highD.shape = (batch, int(tileSize*upRes), int(tileSize*upRes), high.shape[4])
			
		# note - outputs all channels as images, also vel channels...
		clout = np.arange(low.shape[4])
		savePngsGrayscale(lowD, path+'low_'+suff, imageCounter =batchCounterGlob, tiles_in_image=[batch//tileX,tileX], channels=clout )
		chout = np.arange(high.shape[4])
		savePngsGrayscale(tiles=highD, path=path+'high_'+suff, imageCounter=batchCounterGlob, tiles_in_image=[batch//tileX,tileX], channels=chout )

	# plot velocities , for individual samples
	if save_vels:
		#print("XXXa "+format(low[0,1,1,1])); print("XXXb "+format(low[1,1,1,1])) # NT_DEBUG
...
def gridInterpolBatch(macgridbatch, targetshape, order=1):
	...
	mactargetbatch = scipy.ndimage.map_coordinates(macgridbatch, [b, z * fz, y * fy, x * fx, c], order=order, mode='nearest')
...
def getSemiLagrPosBatch(macgrid_batch, dt, cube_len_output=-1):  # check interpolation later
	...
	factor_x = float(_xn) / cube_len_output
	factor_y = float(_yn) / cube_len_output
	factor_z = float(_zn) / cube_len_output
	x_ = np.linspace(0.5, int(_xn / factor_x + 0.5) - 0.5, int(_xn / factor_x + 0.5))
	y_ = np.linspace(0.5, int(_yn / factor_y + 0.5) - 0.5, int(_yn / factor_y + 0.5))
	interp_shape = [_bn, int(_zn / factor_z + 0.5), int(_yn / factor_y + 0.5), int(_xn / factor_x + 0.5), 3]
	...
	if (is3D):
		...
		tarshape = [1, int(_yn / factor_y + 0.5), int(_xn / factor_x + 0.5), 2]
	...
	inter_mac_batch = getMACGridCenteredBatch(inter_mac_batch, is3D) / factor_x
...
def selectRandomTempoTiles(self, selectionSize, isTraining=True, augment=False, n_t=3, dt=0.25):
	'''
	...
	return ori_input_shape.reshape((real_batch_sz, -1)), batch_high.reshape((real_batch_sz, -1)), vel_pos_high_inter #np.tile(batch_high.reshape((real_batch_sz, -1)), [1,2]) # vel_pos_high_inter # 
