...
class TileCreator(object):
	...
	def __init__(self, tileSizeLow, simSizeLow=64, upres=2, dim=2, dim_t=1, overlapping=0, densityMinimum=0.02, premadeTiles=False, partTrain=1.0, partTest=0.0, partVal=0, channelLayout_low=C_LAYOUT['dens_vel'], channelLayout_high=C_LAYOUT['dens'], highIsLabel=False, loadPN=False, padding=0):
		'''
...
def savePngsBatch(low,high, TC, path, batchCounter=-1, save_vels=False, dscale=1., vscale=1., tileSize = 16, upRes=4):
	...
	for axis in range(1 if dim==2 else 3):
		...
		if dim == 2:
			...
			highD.shape = (batch, int(tileSize*upRes), int(tileSize*upRes), 1)
			
		# note - outputs all channels as images, also vel channels...
		clout = np.arange(low.shape[4])
		savePngsGrayscale(lowD, path+'low_'+suff, tiles_in_image=[batch//tileX,tileX], imageCounter = batchCounterGlob, channels=clout )
...
def savePngsGrayscale(tiles, path, imageCounter=0, tiles_in_image=[1,1], channels=[0], save_gif=False, plot_vel_x_y=False, max = 1.0):
	'''
		tiles_in_image: (y,x)
	...
	for image in range(noImages):
		...
		if len(img_c)>2 and plot_vel_x_y:
			scipy.misc.toimage(img_c[0], cmin=0.0, cmax=max).save(path + 'img_{:04d}.png'.format(imageCounter+image))
			saveVel(img, path, imageCounter+image)
			#saveRGBChannels(img,path, [[1,2,3]], imageCounter+image)
		else:
			if len(channels) == 1:
				scipy.misc.toimage(img_c[channels[0]], cmin=0.0, cmax=max).save(path + 'img_{:04d}.png'.format(imageCounter*noImages+image))
			else:
				for i in channels:
					scipy.misc.toimage(img_c[i], cmin=0.0, cmax=max).save(path + 'img_{:04d}_c{:04d}.png'.format(i,imageCounter*noImages+image))
	#TODO save gif
...
def saveRGBChannels(data, path, channel_list, imageCounter=0):
	...
	for i in channel_list:
		...
		img = np.absolute(img)
		scipy.misc.toimage(img, cmin=0.0, cmax=35.0).save(path + 'img_rgb_{:04d}.png'.format(imageCounter))
