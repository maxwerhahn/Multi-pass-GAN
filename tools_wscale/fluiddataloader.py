...
class FluidDataLoader(object):
	...
	def __init__(self, print_info=1, base_path="../data/", base_path_y="../data/", simdirname="sim_%04d/", indices=[], numpy_seed = 17179023,
				...
				conv_slices = False, conv_axis = 0, density_threshold = 0.002, axis_scaling = [1,1,1,1], axis_scaling_y = [0.25,1,1,1], 
				select_random = 1.0, add_adj_idcs = False,
		...
		""" Constructor , arguments:
			...
			base_path_y: path prefix for all sim dirs for high res data
			...
			conv_slices: data has to be 3d -> convert axis into slices
			conv_axis: if conv_slices -> chosen axis for data (transpose if necessary)
			density_threshold: density threshold for discarding slices
			axis_scaling: upsamples converted axis lineraly
			axis_scaling_y: upsamples converted axis lineraly for high res sim
			select_random: selects random frames from left over slices (1.0 -> select all)
		...
		self.conv_axis = conv_axis
		self.density_threshold = density_threshold
		self.axis_scaling = axis_scaling
		self.axis_scaling_y = axis_scaling_y
		self.select_random = select_random
		self.add_adj_idcs = add_adj_idcs
	...
	def getFilename(self, sim_index, fnbase, frame_index, file_path): 
		if not self.oldNamingScheme:
			fn = os.path.join( file_path, os.path.join((self.simdirname % sim_index), (fnbase % frame_index)) )
		else:
	...
	def collectFilenamesFromDir(self, list_index): 
		...
		if self.wildcard is not None:
			...
			for t in range(0,n):
				...
				fn = self.getFilename(sim_index, self.filename, filelist_index, file_path= self.base_path)
				self.xfn.append(fn)
				foundCnt += 1

				if self.filename_y is not None:
					fny = self.getFilename(sim_index, self.filename_y, filelist_index,file_path= self.base_path_y)
					self.yfn.append(fny)
					self.have_y_npz = True # flag to indicate we have np arrays in y
	...
	def removeSlices(self, fx, fy = None):		
		""" 
		If convert 3D data to slices: remove slices below density threshold
		"""
		real_size = 0
		
		fx_out = np.zeros_like(fx)
		if fy is not None:
			fy_out = np.zeros_like(fy)
		for i in range(fx.shape[0]):
			if float(np.average(fx[i,:,:,0:1])) >= self.density_threshold:
				fx_out[real_size] = fx[i]
				if fy is not None:
					fy_out[real_size] = fy[i]
				real_size += 1
		if fy is None:
			return fx_out[0:real_size]
		else:
			return fx_out[0:real_size], fy_out[0:real_size]
			
	def addAdjSlices(self, fx):
		""" 
		adds the adjacent slices as an additional density channel
		"""
		
		fx_shape = fx.shape
		fx = fx.reshape((fx_shape[0], fx_shape[1], fx_shape[2], 3, -1))
		fx_shape = fx.shape
		fx_out = np.zeros((fx_shape[0], fx_shape[1], fx_shape[2], fx_shape[3], fx_shape[4] + 2))
		fx_out[:,:,:,:,0:fx_shape[4]] = fx
		
		for i in range(fx_shape[0]):
			if i == 0:
				fx_out[i:i+1,:,:,:,fx_shape[4]:fx_shape[4]+1] = np.zeros_like(fx[i:i+1,:,:,:,0:1])
				fx_out[i:i+1,:,:,:,fx_shape[4]+1:fx_shape[4]+2] = fx[i+1:i+2,:,:,:,0:1]		
			elif i == fx_shape[0]-1:
				fx_out[i:i+1,:,:,:,fx_shape[4]:fx_shape[4]+1] = fx[i-1:i,:,:,:,0:1]	
				fx_out[i:i+1,:,:,:,fx_shape[4]+1:fx_shape[4]+2]= np.zeros_like(fx[i-1:i,:,:,:,0:1])
			else:
				fx_out[i:i+1,:,:,:,fx_shape[4]:fx_shape[4]+1] = fx[i-1:i,:,:,:,0:1]		
				fx_out[i:i+1,:,:,:,fx_shape[4]+1:fx_shape[4]+2] = fx[i+1:i+2,:,:,:,0:1]	
		return fx_out.reshape((fx_shape[0], fx_shape[1], fx_shape[2], -1))
		
	def selectRandomSamples(self, fx, fy = None):
		""" 
		selects select_random samples from fx / fy
		"""
		rnd_length = int(fx.shape[0] * self.select_random)
		idcs = np.random.shuffle(np.arange(fx.shape[0]))
		if fy is None:
			return (fx[idcs])
		else:
			return (fx[idcs][0])[0:rnd_length], (fy[idcs][0])[0:rnd_length]
	...
	def loadFiles(self):
		...
		for t in range(n):
			...
			# transpoe axis + adjust velocity
			if self.conv_slices:
				if self.conv_axis == 1:
					fx = fx.transpose(1,0,2,3)	
					if fx.shape[3] > 3:
						for i in range(3):						
							fx[:,:,:,i*4 + 2:i*4 + 3], fx[:,:,:,i*4 + 3:i*4 + 4] =  np.copy(fx[:,:,:,i*4 + 3:i*4 + 4]), np.copy(fx[:,:,:,i*4 + 2:i*4 + 3])
						#	fx[:,:,:,i*4 + 1:i*4 + 4] *= np.asarray(self.axis_scaling)[0:3]
						
				elif self.conv_axis == 2:
					fx = fx.transpose(2,1,0,3)
					# swap velocity channels
					if fx.shape[3] > 3:
						for i in range(3):		
							fx[:,:,:,i*4 + 1:i*4 + 2], fx[:,:,:,i*4 + 3:i*4 + 4] = np.copy(fx[:,:,:,i*4 + 3:i*4 + 4]), np.copy(fx[:,:,:,i*4 + 1:i*4 + 2])
						#	fx[:,:,:,i*4 + 1:i*4 + 4] *= np.asarray(self.axis_scaling)[0:3]
						# TODO  UPDATE FILE
			...
			if self.have_y_npz:
				...
				if self.conv_slices:
					if self.conv_axis == 1:
						fy = fy.transpose(1,0,2,3)
					elif self.conv_axis == 2:					
						fy = fy.transpose(2,1,0,3)
			...
			if self.x is None:
				...
				if self.conv_slices:
					# n*self.shape[0] instead of 28000
					self.x = np.zeros( tuple([int(n*self.shape[0]*self.select_random)]+list(self.shape[1:len(self.shape)])) , dtype=FDG_DTYPE)
				else:
			...
			if self.have_y_npz:
				...
				if self.y is None:
					...
					if self.conv_slices:					
						self.y = np.zeros( tuple([self.x.shape[0]]+list(self.shape_y[1:len(self.shape_y)])) , dtype=FDG_DTYPE)
					else:
			...
			if self.conv_slices:
				if self.have_y_npz:
					fx = scipy.ndimage.zoom( fx, self.axis_scaling, order=1)
					fy = scipy.ndimage.zoom( fy, self.axis_scaling_y, order=1)
					if self.add_adj_idcs:
						fx = self.addAdjSlices(fx)
					fx, fy = self.removeSlices(fx, fy)
					fx, fy = self.selectRandomSamples(fx, fy)		
				else:
					fx = scipy.ndimage.zoom( fx, self.axis_scaling, order=1) 
					if self.add_adj_idcs:
			...
			if self.have_y_npz:
				if self.conv_slices:
					self.y[true_n:true_n + fy.shape[0],:] = fy
				else:
					self.y[t,:] = fy
			if self.conv_slices:
				true_n += fx.shape[0]
		...
		if self.print_info and self.conv_slices: print("Removed " + format(self.x.shape[0]-true_n) + " slices by checking against the density_threshold " + format(self.density_threshold) + " and randomly selecting " + format(self.select_random) + " percent of the remaining frames.")
		if self.conv_slices:
			self.x = self.x[0:true_n]
			self.y = self.y[0:true_n]
